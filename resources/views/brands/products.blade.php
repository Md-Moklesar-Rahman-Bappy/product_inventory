...
@section('title', 'Products in ' . $brand->brand_name)
            ...
            <div class="card-header bg-primary text-white py-3">
                <div class="row gy-3 align-items-center">
                    ...
                    <div class="col-12 col-lg-3 d-flex align-items-center">
                        <h5 class="mb-0 fw-bold"><i class="bi bi-box-seam me-2"></i>Products in {{ $brand->brand_name }}</h5>
                        <span class="badge bg-light text-dark ms-2">{{ $products->total() }}</span>
                    </div>

                    {{-- Search --}}
                    <div class="col-12 col-lg-4">
                        <form method="GET" action="{{ route('brands.products', $brand->id) }}" class="d-flex gap-2">
                    ...
                    <div class="col-12 col-lg-5 text-lg-end">
                        @if(auth()->user()->permission <= 1)
                            <div class="d-flex flex-wrap justify-content-lg-end gap-2">
                                <a href="{{ route('products.create', ['brand_id' => $brand->id]) }}" class="btn btn-sm btn-warning fw-bold">
                                    <i class="bi bi-plus-lg me-1"></i>Add Product
                                </a>
                                <a href="{{ route('products.export.brand', $brand->id) }}" class="btn btn-sm btn-light text-success">
                                    <i class="bi bi-file-earmark-excel me-1"></i>Export
                                </a>
                            </div>
                        @endif
                        <a href="{{ route('brands.index') }}" class="btn btn-sm btn-outline-light mt-2 mt-lg-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <small class="text-muted">{{ $p->category->category_name ?? '' }} | {{ $p->model->model_name ?? '' }}</small>
                                </td>
                            ...
                            <tr>
                                <td colspan="8" class="text-center py-4">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        @if(auth()->user()->permission <= 1)
                                            <a href="{{ route('products.create', ['brand_id' => $brand->id]) }}" class="btn btn-primary btn-sm mt-2">
                                                <i class="bi bi-plus-lg me-1"></i>Add Product
                                            </a>
                                        @endif
                                    </div>
